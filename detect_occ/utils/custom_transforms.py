...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        occ_ori = np.array(labels[1]).astype(np.float32)
        occ_edge = np.array(labels[0]).astype(np.float32)
        labels[1] = torch.from_numpy(occ_ori).float()
        labels[0] = torch.from_numpy(occ_edge).long()
...
class OrderRandomHorizontalFlip(object):
    ...
    def __call__(self, sample):
        """
        :param sample: 'image'; 'label':edge[0,1], ori[-pi,pi]
        """
        img   = sample['image']
        masks = sample['label']  # [label_edge, label_edge]
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if short_size < self.crop_size:
            ...
            labels[0] = ImageOps.expand(labels[0], border=(0, 0, padw, padh), fill=self.edge_fill_lbl)
            labels[1] = ImageOps.expand(labels[1], border=(0, 0, padw, padh), fill=self.ori_fill_lbl)
